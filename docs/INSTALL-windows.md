...
압축을 풀면 `climstat-2026-main` 폴더가 생깁니다. 그 폴더를 열었을 때
**`env`, `notebooks`, `data` 폴더가 바로 보여야** 합니다.
같은 이름의 폴더가 한 겹 더 들어 있다면, 안쪽 폴더를 꺼내서 쓰세요.
(압축 프로그램에서 "폴더에 풀기" 를 쓰면 이렇게 됩니다.)

압축을 푼 폴더는 아래 경로로 **옮겨 주세요.**

```
C:\Users\Public\
```

파일 탐색기를 열고(**Windows + E**) 주소창에 위 경로를 붙여넣으면 그 폴더로 바로 갑니다.
거기에 실습 폴더를 끌어다 놓으시면 됩니다.

> **이미 바탕화면에 두신 분도 위 경로로 옮겨 주세요.**
> 처음에는 바탕화면을 안내드렸는데, 그 경로에 한글이 들어가면
> NetCDF 자료를 읽지 못하는 문제가 있어 위치를 바꾸었습니다.
> **폴더만 옮기시면 되고, 환경(climstat)은 다시 만들지 않아도 됩니다.**
...
### Check 4 에서 `FileNotFoundError`

파일은 제자리에 있는데도 못 찾는다고 나오는 경우입니다.
**실습 폴더 경로에 한글이 들어 있어서** 생기는 문제이니, 잘못하신 것이 아닙니다.

에러 메시지에 아래처럼 한글이 보이면 이 경우입니다.

```
C:\Users\user\OneDrive\문서\바탕 화면\...
```

실습 폴더를 `C:\Users\Public\` 아래로 옮긴 뒤, 프롬프트에서 다시 실행하세요.

```
cd C:\Users\Public\climstat-2026
conda activate climstat
jupyter-lab
```
...
노트북을 저장하고 `.ipynb` 파일을 조교에게 보내 주세요.
오류 메시지가 파일 안에 그대로 저장되어 있어 원인을 바로 확인할 수 있습니다.
